...
// short-circuit emitted by client.go.tmpl. Under TELEGRAM_VERIFY=1
...
func TestClient_VerifyShortCircuit_MutatingVerbs(t *testing.T) {
	for _, verb := range []string{"DELETE", "POST", "PUT", "PATCH"} {
		...
		t.Run(verb, func(t *testing.T) {
			...
			if got, _ := env["__verify_synthetic__"].(bool); !got {
				t.Fatalf("envelope must include __verify_synthetic__: true; got %v", env)
			}
...
func TestClient_VerifyShortCircuit_LiveHTTPOptIn(t *testing.T) {
	t.Setenv("TELEGRAM_VERIFY", "1")
	t.Setenv("TELEGRAM_VERIFY_LIVE_HTTP", "1")
	c, rec := newClientWithRecorder(t)
...
// TELEGRAM_VERIFY=1, because they cannot mutate remote state.
...
// under TELEGRAM_VERIFY=1, because the operation does not mutate
