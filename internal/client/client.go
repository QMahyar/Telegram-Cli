...
const BinaryResponseHeader = "X-Telegram-Binary-Response"
...
// real transport even under TELEGRAM_VERIFY=1 without LIVE_HTTP=1.
...
// TELEGRAM_VERIFY=1 (without LIVE_HTTP=1 opt-in), generated
...
// verify mode. The __verify_synthetic__ sentinel is namespace-
// reserved (no real API uses __verify_*) so downstream consumers
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool, mutationIntent bool) (json.RawMessage, int, error) {
	...
	// that sets TELEGRAM_VERIFY=1) drives a mutating verb or explicit
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "telegram-cli/0.1.0")
		}
...
// sanitizeJSONResponse and emitted as invalid JSON. The _binary
