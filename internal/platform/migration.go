...
func readOnlySQLiteDSN(path string) string {
	if runtime.GOOS == "windows" {
		// modernc.org/sqlite fails with "SQL logic error: out of memory"
		// on mode=ro URL opens on Windows. The platform only ever reads
		// through this DSN, so fall back to a plain path open there.
		return path
	}
