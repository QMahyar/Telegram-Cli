...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("pre_flight_before_any_batch_run",
			mcplib.WithDescription("Confirms every account is authenticated and out of cooldown before fan-out."),
			mcplib.WithBoolean("probe", mcplib.Description("Override the recipe's --probe value.")),
		),
		handlePreFlightBeforeAnyBatchRun,
	)
}

// handlePreFlightBeforeAnyBatchRun runs the pre_flight_before_any_batch_run recipe intent: Confirms every account is authenticated and out of cooldown before fan-out.
func handlePreFlightBeforeAnyBatchRun(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
