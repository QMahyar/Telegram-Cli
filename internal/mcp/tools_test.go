...
func TestMCPRegisterToolsPreservesTypedSpecialTools(t *testing.T) {
	s := server.NewMCPServer("telegram", "test")
	RegisterTools(s)
...
func TestMCPLocalStoreMetaIncludesOldestSyncedAt(t *testing.T) {
	...
	if _, err := db.DB().Exec(`INSERT INTO sync_state(resource_type, last_synced_at, total_count) VALUES (?, ?, ?)`, "mirror", syncedAt, 0); err != nil {
...
func TestMCPToolResultTextBoundsSingleArrayEnvelope(t *testing.T) {
	...
	var envelope struct {
		...
		Truncated     bool              `json:"_mcp_truncated"`
		TotalCount    int               `json:"_mcp_total_count"`
		ReturnedCount int               `json:"_mcp_returned_count"`
	}
	if err := json.Unmarshal([]byte(text), &envelope); err != nil {
