...
// the binding layer's compaction as a truncation reason, and never returns a
// result above the MCP text budget.
func WithMetadata(result string, metadata any) string {
	...
	if obj, ok := payload.(map[string]any); ok {
		mcpTruncated, _ := obj["_mcp_truncated"].(bool)
		if mcpTruncated {
			meta["truncated"] = true
...
func boundedSingleArrayObjectWithNote(data json.RawMessage, note string) ([]byte, bool) {
	...
	build := func(subset []json.RawMessage) any {
		...
		if len(subset) < len(items) {
			...
			out["_mcp_total_count"] = len(items)
			out["_mcp_returned_count"] = len(subset)
			out["_mcp_original_bytes"] = len(data)
			out["_mcp_max_bytes"] = MaxBytes
			out["_mcp_note"] = note
		}
		return out
	}
