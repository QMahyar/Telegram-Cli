...
// stopwords typical of every CLI ("widget", "thing", "today",
...
func TestRecall_PlaybookSurfaces_OnFamilyMatch(t *testing.T) {
	...
	playbookJSON := `{
		...
		"steps": [
			{"cmd": "example-cli items list --query {$ENTITY.canonical}", "purpose": "find item"}
		]
	}`
...
func TestRecall_PlaybookSurfaces_DifferentEntitySameFamily(t *testing.T) {
	...
	playbookJSON := `{
		...
		"expected_tool_calls": 2,
		"steps": [
			{"cmd": "example-cli items list --query {$ENTITY.canonical}"}
		]
	}`
...
func TestRecall_PlaybookCoexistsWithResourceHit(t *testing.T) {
	...
	playbookJSON := `{
		"query_family_examples": ["report Alpha widget today"],
		"entity_slots": ["$ENTITY"],
		"steps": [{"cmd": "example-cli items get A1-{$ENTITY.canonical}"}]
	}`
