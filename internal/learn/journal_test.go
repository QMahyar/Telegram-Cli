...
func withJournalHome(t *testing.T) string {
	...
	t.Setenv("TELEGRAM_VERIFY", "")
	t.Setenv("TELEGRAM_DOGFOOD", "")
	t.Setenv("TELEGRAM_NO_LEARN", "")
	t.Setenv("TELEGRAM_LEARN_NO_CAPTURE", "")
	t.Setenv("TELEGRAM_LEARN_SESSION", "")
	return dir
}
...
func runCLI(t *testing.T, args ...string) error {
	t.Helper()
	oldArgs := os.Args
	os.Args = append([]string{"telegram-cli"}, args...)
...
func TestJournal_SessionKeyFromEnv(t *testing.T) {
	withJournalHome(t)
	t.Setenv("TELEGRAM_LEARN_SESSION", "session-abc123")
	if err := runCLI(t, "version"); err != nil {
		t.Fatalf("version: %v", err)
	}
	entries := readAllJournalEntries(t)
...
func TestJournal_DisableSwitchesWriteNothing(t *testing.T) {
	cases := []struct {
		...
		{name: "verify env", env: map[string]string{"TELEGRAM_VERIFY": "1"}},
		{name: "dogfood env", env: map[string]string{"TELEGRAM_DOGFOOD": "1"}},
		{name: "master no-learn env", env: map[string]string{"TELEGRAM_NO_LEARN": "1"}},
		{name: "capture-only env", env: map[string]string{"TELEGRAM_LEARN_NO_CAPTURE": "1"}},
