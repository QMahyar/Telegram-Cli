...
// telegram-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"telegram-cli/internal/learn/lookups"
	"telegram-cli/internal/learn/patterns"
...
const noLearnEnvVar = "TELEGRAM_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("telegram-cli")
}
...
func newTeachCmd(flags *rootFlags, learnCfg *entities.Config) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		// teach.log); declare it so `CLI verification` scores the
		...
		Annotations: map[string]string{"cli:typed-exit-codes": "0,2", "mcp:local-write": "true"},
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  telegram-cli teach --query "<question>" --resource-type <type> --resource <id> --resource <id> &`,
...
when learnings exist.`,
		Example: `  telegram-cli recall "<question>" --agent
  telegram-cli recall "<question>" --agent --min-confidence 2`,
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  telegram-cli learnings forget "<question>" --resource <id>
  telegram-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  telegram-cli teach-pattern --query-template "items in {entity}" --resource-template "GROUP-{entity:category}" --resource-type "items" --entity-kind "category" --strategy substitute`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  telegram-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
