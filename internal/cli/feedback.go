...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TELEGRAM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TELEGRAM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "telegram-cli/feedback")
...
When ` + "`TELEGRAM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TELEGRAM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  telegram-cli feedback list
  telegram-cli feedback list --limit 5
  telegram-cli feedback list --json`,
