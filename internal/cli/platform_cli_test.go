...
func TestPlatformProviderClientCommandsStillRequireGate(t *testing.T) {
	...
	root := &cobra.Command{Use: "sample-cli"}
...
func TestPlatformArtifactOverridesCannotEscapeSelectedProfile(t *testing.T) {
	session := &platform.Session{
		Profile: &platform.Profile{Name: "tenant-a"}, CLI: "sample-cli",
...
func TestPlatformCredentiallessCLIAndMCPUseEmptyCredentials(t *testing.T) {
	...
	t.Setenv("TELEGRAM_CLIENT_PROFILE", "tenant-a")
...
func TestPlatformCLIConformanceOutputMetadataAndAnalytics(t *testing.T) {
	...
	session, err := platform.PrepareProfileSourceFromConfig(profile, "sample-cli", "test-source")
	if err != nil {
		t.Fatal(err)
	}
...
func TestPlatformMigrationAdoptsOnlyVerifiedTenantDatabase(t *testing.T) {
	...
	backupPath := legacyDB + ".telegram-backup"
	backupInfo, err := os.Stat(backupPath)
	if err != nil || (runtime.GOOS != "windows" && backupInfo.Mode().Perm() != 0o600) {
	...
	paths, err := platform.PathsFor("tenant-adopt", "telegram-cli", "test-source")
	if err != nil {
		t.Fatal(err)
	}
	migratedDSN := "file:" + paths.DataFile + "?mode=ro"
	if runtime.GOOS == "windows" {
		migratedDSN = paths.DataFile // mode=ro URL opens fail on Windows modernc sqlite
	}
	migrated, err := sql.Open("sqlite", migratedDSN)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPlatformMigrationWrongTenantCannotCreateProfile(t *testing.T) {
	...
	paths, err := platform.PathsFor("tenant-rejected", "telegram-cli", "test-source")
	if err != nil {
		t.Fatal(err)
	}
