...
type rootFlags struct {
	...
	// invocation. Mirrors the TELEGRAM_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'telegram-cli profile list')")
	rootCmd.PersistentFlags().StringVar(&flags.clientProfileName, "client-profile", "", "Select the tenant-gated client profile (env: TELEGRAM_CLIENT_PROFILE)")
	...
	rootCmd.AddCommand(newMirrorPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelAccountsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelBatchCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelBroadcastCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelDaemonCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelDigestCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelInboxCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelJobsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSchemaCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSinceCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelStatsCmd(flags))
	// Absorbed commands: Telegram MTProto surface
	rootCmd.AddCommand(newChatsCmd(flags))
	rootCmd.AddCommand(newMessagesCmd(flags))
	rootCmd.AddCommand(newSendCmd(flags))
	rootCmd.AddCommand(newForwardCmd(flags))
	rootCmd.AddCommand(newDeleteCmd(flags))
	rootCmd.AddCommand(newReadCmd(flags))
	rootCmd.AddCommand(newReactCmd(flags))
	rootCmd.AddCommand(newEditCmd(flags))
	rootCmd.AddCommand(newMediaCmd(flags))
	rootCmd.AddCommand(newBlockCmd(flags))
	rootCmd.AddCommand(newUnblockCmd(flags))
