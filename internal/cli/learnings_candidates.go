...
alter command behavior or recall's verified results on their own.`,
		Example: `  telegram-cli learnings candidates
  telegram-cli learnings candidates --class flag_alias
  telegram-cli learnings candidates --status rejected`,
...
Usage errors (unknown id, non-open status) exit 2.`,
		Example: `  telegram-cli learnings confirm 3`,
		// Usage errors here exit 2 by design; declare it so
		// `CLI verification` scores the Execute cell honestly.
...
Usage errors (unknown id, no-reject-path candidates) exit 2.`,
		Example: `  telegram-cli learnings reject 3`,
		// Usage errors here exit 2 by design; declare it so
		// `CLI verification` scores the Execute cell honestly.
		Annotations: map[string]string{"cli:typed-exit-codes": "0,2"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
			...
			if errors.Is(err, store.ErrConfirmedFlagAliasReject) {
				return usageErr(fmt.Errorf("learnings reject: candidate %d is a confirmed flag correction with no reject path; repair the guidance with \"telegram-cli playbook amend\" instead", id))
			}
			if err != nil {
				return usageErr(fmt.Errorf("learnings reject: %w", err))
			}
...
purge also clears stale candidates nobody judged.`,
		Example: `  telegram-cli learnings purge
  telegram-cli learnings purge --tombstones`,
