...
// first leaf that (a) carries the `cli:endpoint` annotation, so it actually
...
func isSuggestableReadLeaf(cmd *cobra.Command) bool {
	...
	// supposed to avoid.
	if cmd.Annotations["cli:endpoint"] == "" {
		return false
	}
...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  telegram-cli doctor
  telegram-cli doctor --json
  telegram-cli doctor --fail-on warn
  telegram-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "telegram-cli", suggestion)
						} else {
			...
			// inherits TELEGRAM_VERIFY=1 (parent shell, CI runner, container
			...
			if cliutil.IsVerifyEnv() {
				if cliutil.IsVerifyLiveHTTPEnv() {
					...
					report["verify_mode"] = "INFO ACTIVE — mutating HTTP verbs short-circuit (TELEGRAM_VERIFY=1; no network calls for DELETE/POST/PUT/PATCH)"
				}
			} else {
