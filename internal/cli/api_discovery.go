		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) > 0 {
				...
				for _, child := range root.Commands() {
					if child.Annotations["cli:api-resource"] == "true" && strings.ToLower(child.Name()) == target {
				...
				return fmt.Errorf("interface %q not found. Run '%s-cli api' to list all interfaces", args[0], "telegram")
			}
			...
			for _, child := range root.Commands() {
				if child.Annotations["cli:api-resource"] == "true" {
