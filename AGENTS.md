# Telegram CLI Agent Guide

This directory is the `telegram-cli` repository. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
telegram-cli doctor --json
telegram-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
telegram-cli which "<capability>" --json
telegram-cli <command> --help
```
...
```bash
telegram-cli <command> --help
telegram-cli <command> --dry-run --agent
```
...
Commands that honor `--data-source auto|local|live` prefer live data with a local fallback; `local` is for local-only reads, `live` for remote-only reads, and `computed` for pure computation from embedded rules. Change a scaffold's `auto` default deliberately when its implementation has a narrower source, and reject incompatible `--data-source` requests with a clear error.
...
3. After answering, always fire `telegram-cli teach --query "<question>" --resource <id> --resource-type <type> &` in the background - teaching is unconditional and is the anchor that triggers playbook synthesis. Teach the structural question with identifiers stripped (no names, emails, phone numbers, account ids); the CLI warns on obvious PII shapes but does not block.
...
Disable the loop with `--no-learn` per-invocation or `TELEGRAM_NO_LEARN=true` for the whole session - useful for deterministic agent flows that don't want a learning row to silently change subsequent query results.
...
resolved credential bytes. Do not edit `internal/platform` package internals;
keep downstream registrations in the preserved hand-authored file under
`internal/cli/`.
