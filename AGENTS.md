# Telegram Printed CLI Agent Guide

This directory is a generated `telegram-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
telegram-pp-cli doctor --json
telegram-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
telegram-pp-cli which "<capability>" --json
telegram-pp-cli <command> --help
```
...
```bash
telegram-pp-cli <command> --help
telegram-pp-cli <command> --dry-run --agent
```
...
3. After answering, always fire `telegram-pp-cli teach --query "<question>" --resource <id> --resource-type <type> &` in the background - teaching is unconditional and is the anchor that triggers playbook synthesis. Teach the structural question with identifiers stripped (no names, emails, phone numbers, account ids); the CLI warns on obvious PII shapes but does not block.
